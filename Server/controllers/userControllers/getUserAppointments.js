...
async function getUserValidAppointments (req, res) {
    let token = req.headers.authorization
    let appointments, username
    ...
    try {
        ...
        appointments = await sql.query('SELECT s.storename, fullname, hour, closed_at, ser.name\
                                        ...
                                        WHERE a.username LIKE $1 AND (closed_at > $2 OR closed_at = $2) AND s.id = a.store AND a.service = ser.id', [username, today]);
    } catch (error) {
...
export default getUserValidAppointments;