...
import background from "../../Photos/image.jpg"
...
const AppointmentsComponent =  () => {
    ...
    //     { date: new Date(2024, 2, 5), // An appointment on March 5, 2024
    //       description: "service",
    //       storeName:"Barberia",
    //       time:"12:00"},
    //     { date: new Date(2024, 2, 10), // An appointment on March 10, 2024
    //       description: "Appointment 2",
    //       storeName:"Barberia",
    //       time:"12:30" },
    //     { date: new Date(2024, 2, 13), // An appointment on March 5, 2024
    //       description: "Appointment 1" ,
    //       storeName:"Barberia",
    //       time:"13:00"},
    //     { date: new Date(2024, 2, 15), // An appointment on March 10, 2024
    //       description: "Appointment 2",
    //       storeName:"Barberia",
    //       time:"13:30" },
    //     { date: new Date(2024, 2, 25), // An appointment on March 5, 2024
    //       description: "Appointment 1" ,
    //       storeName:"Barberia",
    //       time:"14:00"},
    //     { date: new Date(2024, 2, 10), // An appointment on March 10, 2024
    ...
    return(
        <div>
            <div className="text-center shadow" style={{backgroundImage: background, borderRadius:"0 0 1.5rem 1.5rem",paddingBottom:"10px",paddingTop:"5px"}} >
                <h2 className='text-center' style={{fontFamily:"cursive", fontWeight:"bold"}}>Welcome to the Calendar</h2>
            </div>
            <div className='container mx-auto p-4'>
                ...
                <div className="grid gap-2" style={{display:'grid' , gridTemplateColumns: 'repeat(7, 1fr)',}}>
                    {WEEKDAYS.map((day) =>{
                        return <div key={day} className='text-center' style={{fontWeight:"bold"}}> {day}</div>
                    })}
                    {Array.from({length:startingDay}).map((_,index)=>{
                        return <div key={`empty-${index}` } className='border rounded p-2 text-center'/>
                    })}
                    {daysOfMonth.map((day,index) =>{
                        ...
                        return( 
                    <div key={index}
                        className="border rounded p-2 text-center" style={{
                            backgroundColor: isSelectedDay ? '#ADD8E6' : 'white',
                            color: isTodayDate ? 'blue' : '',
                            cursor: 'pointer',}} onClick={() => handleDayClick(day)}
                            >
                            {format(day,"d")}
            ...
            <hr></hr>
                {selectedDay && (
                <div>
                    <h3>Appointments for {format(selectedDay, "MMMM d, yyyy")}</h3>
                    <ul>
                    {appointmentData
                        .filter((appointment) =>
                        ...
                        <li key={index}>
                            <div>
                             {appointment.storeName} - {appointment.description} - {appointment.time}
                            </div>
                        </li>
                        ))}
                    </ul>
